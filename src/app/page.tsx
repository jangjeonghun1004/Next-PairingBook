...
// 별도의 클라이언트 컴포넌트로 useSearchParams를 사용하는 부분 분리
function LoginForm({ onCallbackUrlChange }: { onCallbackUrlChange?: () => void }) {
  ...
  const [callbackUrl, setCallbackUrl] = useState('/stories');

  // 검색 파라미터에서 callbackUrl 설정
  useEffect(() => {
    const url = searchParams?.get('callbackUrl') || '/stories';
    if (url !== callbackUrl) {
      setCallbackUrl(url);
      // onCallbackUrlChange 함수가 있다면 호출만 하고 파라미터는 전달하지 않음
      if (onCallbackUrlChange) onCallbackUrlChange();
    }
  }, [searchParams, callbackUrl, onCallbackUrlChange]);

  // 이미 로그인되어 있으면 callbackUrl 또는 스토리 페이지로 리다이렉트
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 로그인 성공 시 원래 접근하려던 페이지 또는 기본 페이지로 이동
      router.push(callbackUrl);
    } catch (error) {
      // 에러 처리
...
export default function HomePage() {
  const handleCallbackUrlChange = () => {
    // 빈 함수만 전달하여 콜백 인터페이스 유지
    // 실제로는 상위 컴포넌트에서 상태를 관리할 필요가 없음
  };
    ...
    <div className="min-h-screen text-white relative">
      ...
      <div className="min-h-screen flex items-center justify-center">
        <Suspense fallback={<div>Loading...</div>}>
          <LoginForm onCallbackUrlChange={handleCallbackUrlChange} />
        </Suspense>
      </div>
    </div>
  );
